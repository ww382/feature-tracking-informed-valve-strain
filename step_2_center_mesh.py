...
import glob
import numpy as np
from pipeline_utils import compute_average_cell_normal, center_polydata_at_origin, read_polydata, write_polydata

def align_mesh_to_z_axis(input_vtk, output_vtk):
    polydata = read_polydata(input_vtk)

    # Rotate the mean normal to the nearest Z direction (+Z or -Z).
    avg_normal = compute_average_cell_normal(polydata)
    z_plus = np.array([0, 0, 1])
    z_minus = np.array([0, 0, -1])

    # Choose closest Z axis
